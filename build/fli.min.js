...
phina.define("nfc.MainScene",{superClass:"nfc.SceneLoadingScene",frame:0,stage:"arcade",difficulty:1,progress:0,score:0,init:function(t){this.superInit(),t.stage&&(this.stage=t.stage),t.difficulty&&(this.difficulty=t.difficulty);var e,i,s,n,a,o,r,h,l,c,p,d,u,g;this.load([function(t){e=phina.display.ThreeLayer({width:SCREEN_WIDTH,height:SCREEN_HEIGHT}).addChildTo(this);var i=new THREE.DirectionalLight(16777215,1);i.position.set(0,0,30),e.scene.add(i),e.scene.add(new THREE.AmbientLight(6316128)),c=phina.display.CircleShape().addChildTo(this),c.radius=75,c.fill="hsla(0, 0%, 30%, 0.5)",c.stroke=null,c.setPosition(SCREEN_WIDTH-100,SCREEN_HEIGHT-100),p=phina.display.TriangleShape().addChildTo(this),p.fill="hsla(0, 50%, 70%, 0.5)",p.stroke="hsla(0, 0%, 0%, 0.5)",p.strokeWidth=1,p.radius=4,p.setPosition(SCREEN_WIDTH-100,SCREEN_HEIGHT-100),d=phina.ui.Gauge({fill:"rgba(0, 0, 0, 0)",gaugeColor:"rgba(255, 64, 64, 0.3)",value:1e3,maxValue:1e3,strokeWidth:1,width:128,height:16}).addChildTo(this),d.animation=!1,d.setPosition(80,SCREEN_HEIGHT-100),u=phina.ui.Gauge({fill:"rgba(0, 0, 0, 0)",gaugeColor:"rgba(64, 64, 255, 0.3)",value:1e3,maxValue:1e3,strokeWidth:1,width:128,height:16}).addChildTo(this),u.animation=!1,u.setPosition(80,SCREEN_HEIGHT-80),g=phina.display.Label({text:"speed: 1",fontSize:16}).addChildTo(this),g.setPosition(50,SCREEN_HEIGHT-20),t()},function(t){i=nfc.EnemyManager(this,e.scene).addChildTo(this),s=i.effectmanager,n=nfc.BulletManager(e.scene).addChildTo(this),a=nfc.WindManager(e.scene).addChildTo(this),t()},function(t){o=phina.asset.AssetManager.get("threejson","fighter").get(),h=phina.asset.AssetManager.get("threecubetex","skybox").get(),l=new THREE.Mesh(new THREE.PlaneGeometry(1e4,1e4),new THREE.MeshBasicMaterial({map:phina.asset.AssetManager.get("threetexture","plane").get()})),l.rotate(-Math.PI/2,0,0),o.position.y=1e3,o.transparent=!0,o.opacity=.3,o.$safe({speeds:[.15,.3,.5,1],myrot:{x:0,y:0,z1:0,z2:0},row:0,yo:0,v:0,rgc:0,brc:0,sc:0,e:1e3,hp:1e3,speed:0,ups:15e-5,av:new THREE.Vector3,update:function(t,e,o){var r=0;if(t.getPointing()&&(this.e--,this.v+=this.speeds[this.speed],r=t.x-SCREEN_CENTER_X,this.myrot.z1+=8e-5*r,this.yo+=8e-5*r,this.row+=(t.y-SCREEN_CENTER_Y)*this.ups),e.getKeyDown(67)&&(this.speed++,this.speed%=4,g.text="speed: "+(this.speed+1)),this.myrot.x+=.1*this.row,this.myrot.y-=.1*this.yo,this.myrot.x%=2*Math.PI,this.quaternion.copy(new THREE.Quaternion),this.rotate((new THREE.Quaternion).setFromAxisAngle(Axis.z,this.myrot.z1+this.myrot.z2)),this.rotate((new THREE.Quaternion).setFromAxisAngle(Axis.x,this.myrot.x)),this.rotate((new THREE.Quaternion).setFromAxisAngle(Axis.y,this.myrot.y)),this.position.addScaledVector(Axis.z.clone().applyQuaternion(this.quaternion).normalize(),this.v),this.position.add(this.av),this.av.multiplyScalar(.98),this.myrot.z1*=.95,e.getKey(16)||Math.abs(this.myrot.x)<Math.PI/2||Math.abs(this.myrot.x)>1.5*Math.PI?(this.ups<15e-5&&(this.ups+=1e-5),this.myrot.z2*=.95):(this.ups>-15e-5&&(this.ups-=1e-5),this.myrot.z2+=.05*(Math.PI-this.myrot.z2)),this.yo*=.95,this.row*=.9,this.v*=.98-6e-5*Math.abs(r)-(e.getKey(66)?.05:0),this.e>0){if(e.getKey(32)&&0===this.sc){this.e-=2;var h=this.quaternion.clone();h.rotate((new THREE.Quaternion).setFromAxisAngle(Axis.x,.06*Math.random()-.03)),h.rotate((new THREE.Quaternion).setFromAxisAngle(Axis.y,.06*Math.random()-.03));var l=this.quaternion.clone();l.rotate((new THREE.Quaternion).setFromAxisAngle(Axis.x,.06*Math.random()-.03)),l.rotate((new THREE.Quaternion).setFromAxisAngle(Axis.y,.06*Math.random()-.03)),this.attack(h,o),this.attack(l,o)}e.getKeyDown(65)&&0===this.rgc&&this.e>=250?(this.rgc=20,this.rgl=2,this.e-=250,this.beam(40,2,15,0,o),s.ray(this,16777215,.2,1,27,7),s.ray(this,65535,.2,2,27,5),s.ray(this,255,.2,4,27,3)):this.rgl>0&&(this.rgl--,this.beam(30,2,15,0,o)),e.getKeyDown(83)&&0===this.brc&&this.e>=700?(this.brc=250,this.brl=17,this.rgc=80,this.sc=50,this.e-=700,this.beam(100,3,25,30,o),s.ray(this,16777215,.2,8,27,24),s.ray(this,16764108,.2,12,27,22),s.ray(this,16746632,.2,18,27,20),s.ray(this,16729156,.2,24,27,18),s.ray(this,16711680,.2,30,27,16)):this.brl>0&&(this.brl--,this.beam(25,3,25,30,o))}this.rgc>0&&this.rgc--,this.brc>0&&this.brc--,this.sc>0&&this.sc--,u.value=this.e,this.e<1e3&&(this.e+=4),d.value=this.hp;for(var c=0;c<a.elements.length;c++){var p=15+a.get(c).size,m=Math.sqrt(this.position.x*a.get(c).position.x+this.position.z*a.get(c).position.y);p>=m&&(this.av.y+=a.get(c).v/2)}for(var c=0;c<n.elements.length;c++){var y=Axis.z.clone().applyQuaternion(this.quaternion).setLength(54),E=Axis.z.clone().applyQuaternion(n.get(c).quaternion).setLength(n.get(c).size),f=this.position.clone().sub(y.clone().multiplyScalar(-.5)),x=n.get(c).position.clone().sub(E.clone().multiplyScalar(-.5));nfc.colCup2D3(f,x,y,E,15+n.get(c).size)&&(s.explode(n.get(c).position,n.get(c).size,10),this.hp-=n.get(c).atk*o.difficulty,n.remove(c))}for(var c=0;c<i.elements.length;c++){var y=Axis.z.clone().applyQuaternion(this.quaternion).setLength(54),E=Axis.z.clone().applyQuaternion(i.get(c).quaternion).setLength(i.get(c).size),f=this.position.clone().sub(y.clone().multiplyScalar(-.5)),x=i.get(c).position.clone().sub(E.clone().multiplyScalar(-.5));nfc.colCup2D3(f,x,y,E,15+3*i.get(c).size)&&(s.explode(i.get(c).position,i.get(c).size,30),this.hp-=50*i.get(c).hp*o.difficulty/this.v,i.remove(c))}},attack:function(t,e){var n=new THREE.Raycaster;n.set(this.position.clone(),Axis.z.clone().applyQuaternion(t).normalize());var a=n.intersectObjects(i.elements);0!==a.length&&(s.explode(a[0].point,1,10),a[0].object.hp-=5/e.difficulty)},beam:function(t,e,n,a,o){var r=Axis.z.clone().applyQuaternion(this.quaternion).normalize();if(0===a)var h=new THREE.Raycaster(this.position.clone(),r).intersectObjects(i.elements);else for(var h=[],l=0;l<i.elements.length;l++){var c=this.position.clone().add(r.multiplyScalar(27)),p=i.get(l).position,d=c.addScaledVector(r,c.clone().sub(p).negate().dot(r)/r.clone().dot(r)).sub(p).length();d<=a+5*i.get(l).size&&h.push({point:p.clone(),object:i.get(l)})}for(var l=0;l<h.length;l++)s.explode(h[l].point,e,n),h[l].object.hp-=t/o.difficulty}}),h.update=function(){this.move(o.position)},l.update=function(){this.position.x=o.position.x,this.position.z=o.position.z},t()},function(t){i.defineEnemy("enem1",{v:.6,duration:100,update:function(){this.position.addScaledVector(Axis.z.clone().applyQuaternion(this.quaternion).normalize(),this.v),this.time%this.duration===0&&n.createBullet({position:this.position,quaternion:this.quaternion,v:2,atk:70}),this.quaternion.rotate(this.c),this.time++}},{rep:6,delay:15}),i.defineEnemy("enem2",{hp:45,v:1,size:15,chase:.1,mindist:500,duration:15,explodeTime:30,update:function(){if(!o.position.equals(this.position)){var t=o.position.clone().sub(this.position.clone()),e=this.v*Math.clamp(2*(t.length()-this.mindist)/this.mindist,-1,1);t.normalize(),this.quaternion.slerp((new THREE.Quaternion).setFromAxisAngle(Axis.z.clone().cross(t).normalize(),Math.acos(Axis.z.clone().dot(t))),this.chase),this.position.addScaledVector(t,e)}this.time%this.duration===0&&n.createBullet({position:this.position,quaternion:this.quaternion,v:3,size:1.5,atk:100}),this.time++}},{}),i.defineEnemy("enem3",{hp:500,v:.25,size:30,duration:3,r:.1,explodeTime:30,scale:new THREE.Vector3(2,2,2),update:function(){this.rotate(this.c);var t=Axis.z.clone().applyQuaternion(this.quaternion).normalize();if(this.rotate((new THREE.Quaternion).setFromAxisAngle(t,this.r)),this.position.addScaledVector(Axis.z.clone().applyQuaternion(this.quaternion).normalize(),this.v),this.time%this.duration===0){var e=this.quaternion.clone().rotate((new THREE.Quaternion).setFromAxisAngle(Axis.x,Math.PI/2));n.createBullet({position:this.position,quaternion:this.quaternion.clone().rotate((new THREE.Quaternion).setFromAxisAngle(e,Math.PI*(this.time%(8*this.duration))/this.duration/8)),size:.5,atk:50})}this.time++}},{}),t()},function(t){if("arcade"===this.stage||phina.asset.AssetManager.get("stage",this.stage))t();else{var e=phina.asset.AssetLoader();e.onload=function(e){t()};var i={stage:{}};i.stage[this.stage]="data/stages/"+this.stage+".min.json",e.load(i)}},function(t){if(e.scene.add(o),e.scene.add(h),e.scene.add(l),"arcade"!==this.stage){for(var s=phina.asset.AssetManager.get("stage",this.stage).get(),n=0;n<s.enemys.length;n++)i.createEnemyMulti(s.enemys[n].name,s.enemys[n].option,s.enemys[n].autospawn);for(var n=0;n<s.winds.length;n++)a.createWind({v:s.winds[n].v,position:s.winds[n].position,size:s.winds[n].size},s.winds[n].color);r=new THREE.Mesh(new THREE.IcosahedronGeometry(s.goal.size,2),new THREE.Material),r.move(new THREE.Vector3(s.goal.x,s.goal.y,s.goal.z))}t()},function(t){e.update=function(t){var s=t.pointer,c=t.keyboard;if("arcade"===this.stage){var d=Math.random();if(d>.98&&i.count()<100){if(.995>d)var u="enem1";else if(.9975>d)var u="enem2";else var u="enem3";i.createEnemyMulti(u,{position:new THREE.Vector3(Math.randint(-500,500),Math.randint(500,5e3),Math.randint(-500,500)).add(o.position),quaternion:(new THREE.Quaternion).rotate(Math.random()*Math.PI*2,Math.random()*Math.PI*2,Math.random()*Math.PI*2)},{options:{position:function(){return new THREE.Vector3(10*Math.random()-5,10*Math.random()-5,10*Math.random()-5)}}})}i.count()>50&&i.remove(0)}for(var g=0;g<i.elements.length;g++)i.get(g).position.clone().sub(o.position).length>2e3&&i.remove(g);"arcade"!==this.stage?(this.progress=o.position.clone().dot(r.position)/r.position.clone().dot(r.position),i.flare("frame",{progress:this.progress})):this.difficulty+=1e-4,i.flare("frame"+this.frame),o.update(s,c,this),h.update(),l.update(),a.flyerposy=o.position.y,e.camera.quaternion.copy(new THREE.Quaternion),e.camera.rotate((new THREE.Quaternion).setFromAxisAngle(Axis.z,-o.myrot.z2)),e.camera.rotate((new THREE.Quaternion).setFromAxisAngle(Axis.x,-o.myrot.x)),e.camera.rotate((new THREE.Quaternion).setFromAxisAngle(Axis.y,o.myrot.y+Math.PI));var m=Axis.z.clone().applyQuaternion(e.camera.quaternion).negate().setLength(-100);e.camera.position.copy(o.position.clone().add(m)),e.camera.updateMatrixWorld(),p.x=SCREEN_WIDTH-100+o.position.x/10,p.y=SCREEN_HEIGHT-100+o.position.z/10,p.rotation=-o.myrot.y/Math.PI*180;for(var g=0;g<n.elements.length;g++)n.get(g).position.clone().sub(o.position).length>800&&n.remove(g);o.hp<=0&&("arcade"===this.stage?this.exit("gameover",{score:this.score}):this.exit("gameover",{score:this.score})),this.frame++}.bind(this),t()}])}});
